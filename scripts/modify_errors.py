...
custom_errors = '''
  // display ERRORS
  DISPLAY_NOT_FOUND,                // The display was not found
'''
...
try:
  ...
  with open(FILEPATH_ERRORS_H, "r") as f:
    ...
    if any(custom_errors.split('\n')[1].strip() in line for line in lines):
      print(info + '\'errors.h\' already contains display custom errors')
    else:
      with open(FILEPATH_ERRORS_H, "w") as f_write:
        for line in lines:
          ...
          f_write.write(line)
      print(info + '\'errors.h\' updated with display custom errors')
